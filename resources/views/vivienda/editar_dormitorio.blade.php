...
<body>
    <div class="container-fluid">
        ...
        <section class="container">
            <header>12.d Dormitorio</header>
            <form id="dormitorioForm" class="form">
                <div class="table-responsive">
                    <table class="table table-bordered" style="width: 100%">
                        ...
                        <tbody>
                            @foreach ($dormitorio as $fila)
                                <tr>
                                    <td>{{ $fila->detalle }}</td>
                                    <td>
                                        <input type="radio" name="respuesta_{{ $fila->cod_dormitorio }}"
                                            value="Si" {{ $fila->respuesta === 'Si' ? 'checked' : '' }} disabled>
                                    </td>
                                    <td>
                                        <input type="radio" name="respuesta_{{ $fila->cod_dormitorio }}"
                                            value="No" {{ $fila->respuesta === 'No' ? 'checked' : '' }} disabled>
                                    </td>
                                    <td>
                                        <textarea class="form-control"name="acciones_{{ $fila->cod_dormitorio }}" disabled>{{ $fila->acciones }}</textarea>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        document.getElementById('dormitorioForm').addEventListener('submit', async (event) => {
            event.preventDefault(); // Evita el envío predeterminado del formulario

            const cod_familia = "{{ $dormitorio->first()->cod_familia ?? '' }}";
